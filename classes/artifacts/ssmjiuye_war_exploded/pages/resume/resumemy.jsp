...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/resumemy">
...
<td>
  <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/resumeedit?id=${c.id}&forword=resumemy" class="btn btn-default">修改</a> |
 <a href="resumedel?id=${c.id}&forword=resumemy" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
  </c:if>
 </td>
</tr></tbody>
	       ...
	       function go(index)
           {
               window.location.href="<%=path %>/resumemy?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
