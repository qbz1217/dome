...
<table   class="table table-hover">
 <tr> <th>
<form name="formsea" class="form-inline" role="form" method="post" action="<%=path %>/resumelist">
  ...
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="jiguan" id="jiguan" class="form-control" placeholder="籍贯"/>
	</div>
  </div>
  ...
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="school" id="school" class="form-control" placeholder="所在院校"/>
	</div>
  </div>
...
<c:forEach items="${zleibie}" var="s"  varStatus="ss">
  ...
  <div class="form-group">
    <div class="col-sm-2">
	<input type="text" name="yuexin" id="yuexin" class="form-control" placeholder="月薪要求"/>
	</div>
  </div>
...
<table class="table table-hover table-striped table-bordered">
 <tr><th>简历列表 &nbsp; &nbsp;
 <!--
 <button type="button" onclick="location.href='<%=path %>/resumeadd'" class="btn  btn-primary">添加</button>
 -->
</th></tr></table>
<table class="table table-hover table-striped table-bordered">
 <thead>
...
<td>${c.jingli}</td>
<td><a href="<%=path %>/resumeview?id=${c.id}" class="btn-default" target="_blank">查看详情</a></td>
<td>${c.hits}</td>
...
<td> 
 <c:if test="${c.zt=='未审核'}">
 <a href="<%=path %>/resumeupdate?id=${c.id}&zt=已审核&forword=resumelist" class="btn btn-info">审核</a> |
 </c:if>
 <a href="resumedel?id=${c.id}&forword=resumelist" onClick="return confirm('您确定要删除吗?')" class="btn btn-default">删除</a>
 </td>
</tr></tbody>
</c:forEach> </table><script type="text/javascript">
	       function go(index)
           {
               window.location.href="<%=path %>/resumelist?index="+index;
           }
</script>
<table class="table table-hover table-striped table-bordered">
						        <tr>
